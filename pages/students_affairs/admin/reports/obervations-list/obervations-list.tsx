...
import { getObservsHall } from "../../../../../Helpers/observs-halls-print";
interface IExamsListProps {}
const ObservationsList: React.FC<IExamsListProps> = ({}) => {
  ...
  const print = () => {
    if (toPrint) {
      ...
      printWindow.document.write(
        getObservsHall(toPrint, startDate, endDate, DateHelper.getCurrentDate())
      );
  ...
  const show = () => {
    ...
    ExamService.observations(st, en)
      .then((resp) => {
  ...
  return (
    <GridContainer md={12}>
      <Grid container md={12} style={{ margin: "2em 0em" }}>
        ...
        <GridItem md={12}>{renderObserv()}</GridItem>
      </Grid>
    </GridContainer>
  );
};

(ObservationsList as any).auth = true;
(ObservationsList as any).layout = Admin;
