...
export const getStudentToPrint = (student, eduYear, type) => {
  ...
  const formatDate = (date) => {
    if (date == null) return '';
    let arabicDate = ConvertToArabicNumbers(date);
    let year = arabicDate.split('-')[0];
    let month = arabicDate.split('-')[1];
    let day = arabicDate.split('-')[2];
    return year + '/' + month + '/' + day;
  }

  const title1 = type == 1 ? "مصدقة دراسية" : "وثيقة دوام";
  const title2 = type == 1 ? "خاصة بالمكلفين بخدمة العلم" : "";
  const pageTitle = type == 1 ? title1 + '' + title2 : title1;
  const headerMessage = `تشهد إدارة المعهد العالي للعلوم التطبيقية والتكنولوجيا أن الطالب المذكور أدناه مسجل في ${student.study_year} ولا يزال يتابع دراسته الجامعية للعام الدراسي ${ConvertToArabicNumbers(eduYear)}، وفق ما يلي:`;
  const footerMessage = type == 1 ?
    "وقد بينا هذه المعلومات من أجل تأجيل خدمته الإلزامية بصفته طالباً يتابع دراسته الجامعية. علماً أن مدة الدراسة في المعهد العالي هي خمس سنوات." :
    (student.moufad ?
      "وهو يتقاضى راتباً شهرياً كونه طالباً موفداً ببعثة دراسة داخلية. علماً أن مدة الدراسة في المعهد العالي هي خمس سنوات."
    :
      "علماً أن مدة الدراسة في المعهد العالي هي خمس سنوات."
    );
      
  const current = formatDate(DateHelper.getCurrentDate().split(' ')[1]);
  const editDateMsg = 'حرر بتاريخ ' + current.split('/')[2] + '/' + current.split('/')[1] + '/' + current.split('/')[0];
...
<div class="text-center">
  <div class="row">
    <div class="col-xs-5" style="text-align: right;">
      <h4 style="text-align: right;">رقم /  ${ConvertToArabicNumbers(student.personal_number)}  /</h4>
    </div>
...
<table>
    ...
    <tr>
        ...
        <td>${student.birth_place} ${formatDate(student.birth_date)}</td>
    </tr>
    <tr>
        <td>محل ورقم القيد</td>
        <td>${ConvertToArabicNumbers(student.qaid_place)}/${ConvertToArabicNumbers(student.qaid_number)}</td>
    </tr>
    <tr>
        <td>شعبة التجنيد</td>
        <td>${student.military_office}</td>
    </tr>
</table>
</div>
<div class="row">
...
<div style="text-align: center;">
  <h4"><b>${editDateMsg}</b></h4>
</div>
<div style="display: flex; flex-direction: row; justify-content: space-between;">
  <h4><b>رئيس دائرة التسجيل والمتابعة</b></h4>
  <h4><b>مدير شؤون الطلاب</b></h4>
  <h4><b>أمين المعهد العالي</b></h4>
</div>
</div>
  `;

  const hiast = 'HIAST';
  const phone = ConvertToArabicNumbers('+963 11 5140520');
  const fax = ConvertToArabicNumbers('+963 11 2237710');
  const pobox = ConvertToArabicNumbers('/31983/');
  const website = 'www.hiast.edu.sy';
  const email = 'hiast@hiast.edu.sy';
  const line1 = `المعهد العالي للعلوم التطبيقية والتكنولوجيا ${hiast} دمشق - مساكن برزة مسبقة الصنع - خلف مشفى حاميش`;
  const line2 = `هاتف ${phone} فاكس ${fax} صندوق بريد ${pobox}`;
  const line3 = `موقع الإنترنت ${website} - البريد الإلكتروني ${email}`;
  const separator = `
  <div style="margin-top: 40px; text-align: center; line-height: 60%;">
  <hr style="border: 1px dashed black; margin-bottom: 10px;" />
  <p style="margin-top: 0;">${line1}</p>
  <p style="margin-top: 0;">${line2}</p>
  <p style="margin-top: 0;">${line3}</p>
